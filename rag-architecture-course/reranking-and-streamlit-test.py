...
def load_documents():
    ...
    """
    caminhos = [
        "rag-architecture-course/documents/politica_ferias.pdf",
        "rag-architecture-course/documents/politica_home_office.pdf",
        "rag-architecture-course/documents/codigo_conduta.pdf"
    ]
...
def generate_chunks(documentos):
    """
...
def rerank_documents(pergunta, documentos, llm):
    """
...
def answer_question(pergunta, vectorstore):
    """
    Pipeline completo:
    - Recuperação
    - Reranking
    ...
    # Reranking
    documentos_rerankeados = rerank_documents(
        pergunta,
...
if pergunta:
    with st.spinner("Consultando políticas internas..."):
        ...
        chunks = generate_chunks(documentos)
        chunks = enrich_chunks(chunks)
        vectorstore = create_vectorstore(chunks)

        resposta, fontes = answer_question(pergunta, vectorstore)
